...
export const checkText = (options = {}) => {
  return (control) => {
    try {
      ...
      const pattern = allowDotsAndCommas
        ? /^[a-zA-Z0-9_.,-\s]{2,100}$/
        : /^[a-zA-Z0-9_-\s]{2,100}$/;
  ...
  return (control) => {
    try {
      ...
      return pattern.test(control.value)
        ? null
        : {
            'Password is weak. Use at least 8 characters, one lowercase, one uppercase letter and a number.': true,
          };
    } catch (error) {
      return null;
    }
...
export const checkAddress = (allowEmpty = true) => {
  return (control) => {
    try {
      ...
      const web3 = new Web3();
      return web3.utils.isAddress(control.value)
        ? null
        : { 'Public key is invalid': control.value };
    } catch (e) {
